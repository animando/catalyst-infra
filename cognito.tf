...
resource "aws_cognito_user_pool_client" "catalyst_cognito_client_app" {
  ...
  callback_urls = ["https://${aws_cloudfront_distribution.ui_cloudfront_distribution.id}"]
  logout_urls = ["https://${aws_cloudfront_distribution.ui_cloudfront_distribution.id}"]
